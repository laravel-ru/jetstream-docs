...
Если Ваше приложение использует стек Inertia, Вы можете вернуть страницы Inertia из Ваших закрытий настройки представления:
...
Если Вы предпочитаете инкапсулировать свой собственный процесс аутентификации в классе, а не в закрытии, Вы можете передать "вызываемый" массив PHP методу `authenticateUsing`:
...
Чтобы определить свой собственный конвейер, Вы можете использовать метод `Fortify::authenticateThrough`. Этот метод принимает закрытие, которое должно возвращать массив классов для передачи запроса входа в систему. Обычно этот метод следует вызывать из метода `boot` Вашего класса `App\Providers\JetstreamServiceProvider`.
...
В дополнение к шаблонам представлений и действий, связанных с входом в систему / аутентификацией, Jetstream также формирует ресурсы, необходимые пользователю для сброса своего пароля в случае его забывания. Конечно, эта функция использует базовые [функции сброса пароля](https://laravel.com/docs/passwords).
